...
> **Auth**: every `/api/*` route requires `Authorization: Bearer <ARRA_API_TOKEN>` when
> `ARRA_API_TOKEN` is set. **Required for any non-local/production deployment** (e.g. Railway) —
> without it the server binds to `127.0.0.1` only (loopback, auth disabled) so plain local dev
> keeps working but nothing is reachable from the network.
