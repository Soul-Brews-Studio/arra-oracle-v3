...
import { apiUrl, fetchVectorConfig, reloadVectorConfig, updateVectorCollection, withOracleFetchInit } from '../api';
...
async function testVectorCollection(key: string): Promise<{ success?: boolean; count?: number; error?: string; status?: string }> {
  const response = await fetch(apiUrl(`/api/v1/vector/config/${encodeURIComponent(key)}/test`), withOracleFetchInit({ method: 'POST', headers: { accept: 'application/json' } }));
